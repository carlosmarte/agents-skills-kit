...
A skill MUST be encapsulated within a single parent directory. The directory name MUST be a non-empty identifier following the same character rules as `name` (§3.1) and MUST perfectly match the `name` field defined in the skill's frontmatter.

The directory name MAY include a stable namespace prefix shared across a related set of skills (e.g., `agent-skill-kit-validator`, `agent-skill-kit-creator`) so that `skills-ref add`-style installers can fetch the set as a coherent unit; the prefix is a naming convention, not a spec requirement.

The `tier` frontmatter value is informational metadata used by the dependency-direction rules in §3.3; it is no longer encoded in the directory name. Skills authored prior to v1.1 of this spec used a `<tier>-<slug>` directory-name format; that form remains valid but is no longer required.

Examples: `.agents/skills/agent-skill-kit-validator/`, `.agents/skills/gap-analysis-agent/`, `.agents/skills/ployglot-scaffold/`.
...
| ----- | -------- | ------------------------------ |
| `name` | **Yes** | 1 to 64 characters long. Allowed characters: Unicode lowercase alphanumeric (`a-z`, `0-9`) and hyphens (`-`). Cannot start or end with a hyphen. Cannot contain consecutive hyphens (`--`). Must exactly match the parent directory name (§2.1). The directory/name pair MAY embed a stable namespace prefix; no tier prefix is required. |
